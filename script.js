...
//Screenshot
const screenShot = document.querySelector(".withIcon:first-of-type");
...
function updateGrid() {
    ...
    if (isValueValid(gridSize)) {
        gridDimensions.textContent = parseInt(gridSize);
        emptyTheGrid();
        fillTheGrid();
    }
    else {
...
//Draw
drawingGrid.addEventListener("click", () => {
    if (drawingCheck === true) drawingCheck = false;
    else drawingCheck = true;
...
drawingGrid.addEventListener("mousemove", (e) => {
    ...
    divsArray.forEach((div) => {
        if (counter == 0) lastDiv = null;
        if (e.target === div && drawingCheck) {
            changePenColorDynamically(e);
            if (div != lastDiv) div.style.background = penColor;
            lastDiv = div;
        }
...
screenShot.addEventListener("click", () => {
    window.scrollTo(0, 0);
    html2canvas(document.querySelector("#drawingGrid")).then(canvas => {
